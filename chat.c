...
int set_userinfo(struct userinfo *user, int fd, char *buf, int nread)
{
    if (user[fd].fd == 0) {
        // set user name
        if (nread > 32)
            nread = 32;
        memcpy(user[fd].name, buf, nread - 2);
        user[fd].fd = fd;
        printf("[%d] set name: %s\n", fd, user[fd].name);
        return 1;
    }
...
int main(int argc, char **argv)
{
    ...
    struct userinfo users[FD_SETSIZE];
    ...
    while (select(max_fd, &rfds, NULL, NULL, NULL) >= 0) {
        ...
        if (FD_ISSET(server_fd, &rfds)) {
            ...
            if (new_fd < 0) {
                ...
                if (write(new_fd, "Enter your name:\0", 18) < 0) {
        ...
        for (int fd = 0; fd < FD_SETSIZE; fd++) {
            ...
            if (FD_ISSET(fd, &rfds)) {
                ...
                if (nread < 0) {
                    ...
                    conns[fd] = 0;
                    users[fd].fd = 0;
                    memset(users[fd].name, 0,
                           sizeof(users[fd].name));
                }
                    ...
                    if (set_userinfo(users, fd, buf, nread))
                        continue;
                    char userbuf[64];
                    ...
                    for (int dest_fd = 0; dest_fd < FD_SETSIZE; dest_fd++) {
                        ...
                        if (conns[dest_fd] && dest_fd != fd) {
                            /* write to them */
                            if (write(dest_fd, userbuf, sz) < 0 || write(dest_fd, buf, nread) < 0) {
                                ...
                                conns[dest_fd] = 0;
                                users[dest_fd].fd = 0;
                                memset(users[dest_fd].name, 0,
                                       sizeof(users[dest_fd].name));
                            }
                        }
                    }
                }
